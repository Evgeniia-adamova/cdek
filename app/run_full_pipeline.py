...
Extracted frames are written to the system temp dir (outside OneDrive/cloud
sync folders) to avoid sync-induced slowdowns on Windows.
...
Resume behaviour: each stage is skipped if its primary output file already
exists, so a crashed run restarts from where it left off.
"""
import json
import os
import shutil
...
# Frames go to system temp so OneDrive/Dropbox sync never touches them.
FRAMES_BASE = Path(tempfile.gettempdir()) / "MT_frames"
...
def _setup_dirs(video_id: str):
    ...
    dirs = {
        ...
        "frames": str(FRAMES_BASE / video_id / "extracted_frames"),
    }
...
def main():
    # Suppress OpenCV/FFmpeg "keyframes not correctly marked" noise on stderr.
    ...
    fd_ext_path = os.path.join(dirs["face_detection"], "face_detection_extended.json")
    ...
    if not os.path.isfile(manifest_path):
        ...
        print("  [resume] Frame manifest exists, skipping extraction.")
        step4_out = {"manifest_path": manifest_path}
    ...
    if not os.path.isfile(fd_path):
        ...
        print("  [resume] Face detection exists, skipping.")

    if not os.path.isfile(fd_ext_path):
        run_step3(
            input_path=fd_path,
            output_path=fd_ext_path,
            interval_sec=10.0,
        )
    else:
        print("  [resume] Face detection extended exists, skipping.")
    ...
    if not ogg_path.is_file():
        ...
        print(f"  [resume] Audio exists, skipping extraction.")
    ...
    if not segments_path.is_file():
        ...
        print("  [resume] Transcript exists, skipping STT.")

    # --- Text analysis (diarization + sentiment) ---
    print("\n=== TEXT ANALYSIS (diarization + sentiment) ===")
    diarized_path = Path(dirs["sentiment"]) / "diarized_sentiment.json"
    if not diarized_path.is_file():
        from app.backend.text_analysis import run_diarize_sentiment
        result = run_diarize_sentiment(
            ogg_path,
            segments_path,
            output_json_path=diarized_path,
            output_txt_path=Path(dirs["sentiment"]) / "diarized_sentiment.txt",
        )
    else:
        print("  [resume] Diarization exists, skipping.")
        result = {
            "output_json": str(diarized_path),
            "output_txt": str(Path(dirs["sentiment"]) / "diarized_sentiment.txt"),
        }
    ...
    if os.path.isfile(combined_output_path):
        print("  [resume] Combined emotion exists, skipping.")
        try:
            with open(combined_output_path, encoding="utf-8") as f:
                combined_result = json.load(f)
        except Exception as e:
            print(f"  Warning: could not load existing combined emotion: {e}", file=sys.stderr)
    else:
        try:
    ...
    if os.path.isfile(kpi_output_path):
        print("  [resume] KPI evaluation exists, skipping.")
    else:
        try:
            ...
            if transcript_text.strip():
                ...
                kpi_result = run_kpi_evaluation(
                    ...
                    video_id=video_id,
                    combined_emotion_result=combined_result,
    ...
    try:
        ...
        print(f"  DB warning (finalize): {e}", file=sys.stderr)
    ...
    print(f"    Combined emotion:  {combined_output_path}")
